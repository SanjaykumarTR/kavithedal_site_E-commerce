...
def _cf_create_order(cf_order_id, amount, customer_id, customer_email,
    ...
    try:
        ...
        if resp.status_code >= 400:
            error_msg = f'Cashfree API error: {resp.status_code}'
            try:
        ...
        if 'payment_session_id' not in cf_response:
            ...
            raise ValueError("Cashfree response missing payment_session_id. Response: " + json.dumps(cf_response))
        ...
        raise ValueError(f'Cashfree HTTP error: {exc.response.text if exc.response else str(exc)}')
    except ValueError:
        # Re-raise ValueError for missing payment_session_id
        raise
    except Exception as exc:
        logger.error(f'Cashfree request failed: {exc}')
        raise ValueError(f'Cashfree request failed: {str(exc)}')
...
class CreateOrderView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.error(f'Cashfree order creation failed for order {order.id}: {exc}')
            order.delete()
            return Response(
                {'error': f'Payment gateway error: {str(exc)}'},
            ...
            order.delete()
            return Response(
                {'error': f'Payment gateway error. Please try again. ({str(exc)})'},
                status=status.HTTP_503_SERVICE_UNAVAILABLE,
            )
...
class EbookPurchaseView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.error(f'Cashfree ebook order creation failed for purchase {purchase.id}: {exc}')
            purchase.delete()
            return Response(
                {'error': f'Payment gateway error: {str(exc)}'},
                status=status.HTTP_503_SERVICE_UNAVAILABLE,
...
class CartCheckoutView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            logger.error(f'Cashfree cart order creation failed: {exc}')
            return Response(
                {'error': f'Payment gateway error: {str(exc)}'},
                status=status.HTTP_503_SERVICE_UNAVAILABLE,
            )
...
class CashfreeWebhookView(APIView):
    ...
    using CASHFREE_WEBHOOK_SECRET before processing any event.
    ...
    def post(self, request):
        ...
        if not webhook_secret:
            logger.error('CashfreeWebhookView: CASHFREE_WEBHOOK_SECRET not configured')
            return Response(
